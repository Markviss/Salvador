(() => {
    ...
    modules_mvsModules.popup = new Popup({});
    const popupImg = document.querySelector(".popup__img");
    const popupDescription = document.querySelector(".popup__text");
    window.addEventListener("click", (function(event) {
        if (event.target.hasAttribute("data-popup")) {
            const cardPopup = event.target.closest("div");
            const popupInfo = {
                imgSrc: cardPopup.querySelector("img").getAttribute("src"),
                imgAlt: cardPopup.querySelector("img").getAttribute("alt"),
                rugDescription: cardPopup.querySelector(".rug-description").innerText
            };
            popupImg.setAttribute("src", `${popupInfo.imgSrc}`);
            popupImg.setAttribute("alt", `${popupInfo.imgAlt}`);
            popupDescription.innerText = `${popupInfo.rugDescription}`;
        }
    }));
        ...
        inputFile.forEach((function(el) {
            ...
            const uploadFile = file => {
                ...
                if (fileList && fileList.length > 1) {
                    if (fileList.length <= 4) textSelector.textContent = `Выбрано ${fileList.length} файла`;
                    if (fileList.length > 4) textSelector.textContent = `Выбрано ${fileList.length} файлов`;
